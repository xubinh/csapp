...
- <a href="#6.3">实验思路与总结</a>
  - <a href="#6.3.1">初始分配器方案介绍</a>
  - <a href="#6.3.2">关于评分工具 `./mdriver` 所使用的默认吞吐量配置</a>
  - <a href="#6.3.3">用于自动化测试的脚本</a>
  - <a href="#6.3.4">DEBUG 流水账: `mm_free` 中忘记接住合并函数 `_coalesce` 的返回值</a>
  - <a href="#6.3.5">DEBUG 流水账: 放置函数 `_place` 中对不分割的情况的处理存在错误</a>
  - <a href="#6.3.6">DEBUG 流水账: 忘记在检查程序 `mm_check` 中释放内存</a>
  - <a href="#6.3.7">性能优化相关: 测试样例 `binary-bal.rep` 和 `binary2-bal.rep` 的内存利用率过低</a>
- <a href="#6.4">相关资料</a>
...
#### <a id="6.3.1"></a>初始分配器方案介绍

[待办]

#### <a id="6.3.2"></a>关于评分工具 `./mdriver` 所使用的默认吞吐量配置

评分工具 `./mdriver` 所使用的默认 libc `malloc` 的吞吐量为 600 Kops/sec, 虽然这个吞吐量大小不太能反映真实的机器性能 (例如某次测试 `binary-bal.rep` 时 libc `malloc` 的吞吐量达到了 19228 Kops/sec), 但是设置这个吞吐量目标的目的并不是类似于 "这个目标非常难达到, 只要使得你的程序追上这个目标就说明你的程序是经过非常好的优化的", 而是类似于 "这个目标是平均难度, 不是很容易但也没有多难, 你应该在达到这个吞吐量目标之后转而追求内存利用率的提升并在二者之间找到合适的平衡", 因此尽管该默认值无法反映真实机器速率, 但仍然是符合本实验所要达到的目的 (设计一个在吞吐率和内存利用率之间达到良好平衡的分配器) 的. 实在感觉到不舒服可以在运行 `./mdriver` 时使用 `-l` 选项同时测试本机上的 libc `malloc` 并肉眼比较吞吐率即可 (理论上 libc 的 `malloc` 可能会在执行过程中多做许多工作, 因此本实验中所实现的简单分配器的吞吐量应该很容易超过 libc 的 `malloc` 的吞吐量, 实际测试的结果也证明了这一点).

#### <a id="6.3.3"></a>用于自动化测试的脚本
...
#### <a id="6.3.4"></a>DEBUG 流水账: `mm_free` 中忘记接住合并函数 `_coalesce` 的返回值
...
#### <a id="6.3.5"></a>DEBUG 流水账: 放置函数 `_place` 中对不分割的情况的处理存在错误
...
#### <a id="6.3.6"></a>DEBUG 流水账: 忘记在检查程序 `mm_check` 中释放内存
...
#### <a id="6.3.7"></a>性能优化相关: 测试样例 `binary-bal.rep` 和 `binary2-bal.rep` 的内存利用率过低

测试时发现测试样例 `binary-bal.rep` 和 `binary2-bal.rep` 的内存利用率过低, 仅为 55% 左右.
...
由于当前的分配器实现方案采用的是 "申请后分配前执行合并" 的策略, 之前分配块时剩下的多余空间在本次分配时被合并到所申请的新的块中, 因此导致本次分配的块恰好在紧邻着前一个分配的块结束的地方开始, 即当前的分配器实现方案在 `binary-bal.rep` 的请求模式下等价于简单的线性分配策略, 因此最后得到的内存利用率也位于上述理论值的附近 (加上头部以及对齐造成的额外空间分配, 实际的内存利用率约为 55%).
...
可以通过微调分配器中的如下代码中的超参求解针对当前给出的 trace file 的最佳参数:
...
其中超参从 5 到 6 时测试样例 `binary2-bal.rep` 的结果发生突变, 这是因为 `binary2-bal.rep` 中采用的是 (16, 112) 的组合, 其中 16 字节的载荷生成 24 字节的块大小, 112 字节的载荷生成 120 字节的块大小, 而超参等于 6 时为 24 字节的块多分配的 5 个块恰好组成一个 120 字节的空闲块, 从而导致最终堆内的分配布局又回到了大小块交叉而外部碎片巨多的情况. 在超参从 7 到 8 时 `binary-bal.rep` 结果的突变同理. 最终选择 5 作为超参的值.
