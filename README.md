...
首先利用 `sdriver.pl` 和 `Makefile` 编写了一个自动化测试的 bash 脚本 `run.sh`, 可以通过命令 `./run.sh test 1` 或 `./run.sh rtest all` 自动测试并将结果输出至文本文件 `output_test.txt` 和 `output_rtest.txt` 中.

- 输出结果只能通过开两个标签页然后使用 `Ctrl+Tab` 快速翻页来肉眼检查.
- 不失一般性, 最终输出结果中删除了程序 `ps` 的输出结果.

其次 `tsh.c` 中已经实现的函数归纳如下 (按调用顺序拓扑排序, 未调用的函数按出现顺序排序):
...
最后除了已经在 `tsh.c` 中实现的骨架, 为了实现完整的 tsh 还需要填充下列 7 个重要函数 (括号中数字代表预计代码行数 (包括注释)):
...
- C 语言语法摘要 (K&R)
  ...
  - P76: 初始化
    ...
    - 对自动变量的初始化其实只是对赋值语句的简写, 选择哪种风格完全是个人口味的问题. K&R中一般使用赋值语句而不是显式初始化, 因为他们认为对变量在声明时初始化离用这个变量的地方太远, 找起来不太方便.
