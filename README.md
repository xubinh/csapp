...
`tsh.c` 为本实验需要实现的 tsh 的源文件, 其中包含一个 shell 的简单骨架以及一些已经实现的简单函数.
...
- 一个命令行 (command line) 是指一个由空格分隔的 ASCII 字符序列, 序列中第一个单词为某个内置 shell 命令或是某个可执行文件的路径, 剩下的所有单词均为参数.
...
除了已经在 `tsh.c` 中实现的骨架, 为了实现完整的 tsh 还需要填充下列函数 (括号中数字为所需代码的预估行数):

- `eval`: 爬取, 解析并执行命令行. [70 行]
- `builtin_cmd`: 识别并执行内置命令, 包括 `quit`, `fg`, `bg` 和 `jobs`. [25 行]
- `do_bgfg`: 内置命令 `bg` 和 `fg` 的具体实现. [50 行]
- `waitfg`: 等待一个前台作业结束. [20 行]
- `sigchld_handler`: 信号处理程序, 负责接收 `SIGCHLD` 信号. [80 行]
- `sigint_handler`: 信号处理程序, 负责接收 `SIGINT` (ctrl-c) 信号. [15 行]
- `sigtstp_handler`: 信号处理程序, 负责接收 `SIGTSTP` (ctrl-z) 信号. [15 行]

关于本实验的一些建议与提示:

- 一字不落地阅读 CS:APP 第 8 章的内容.
- 顺着 trace 文件来开发 tsh, 例如首先从 `trace01.txt` 开始, 确保所实现的 tsh 和 tsh 的参考实现的输出一致, 然后继续 `trace02.txt`, 以此类推.
...
tsh 的参考实现程序. 需要保证所实现的 tsh 与 tsh 的参考实现的输出完全一致.
