...
- <a href="#1">1. Data Lab</a><a href="#1"></a>
  ...
  - <a href="#1.3">思路与总结</a><a href="#1.3"></a>
  ...
  - <a href="#1.4">相关资料</a><a href="#1.4"></a>
    - <a href="#1.4.1">命题逻辑</a><a href="#1.4.1"></a>
...
#### <a id="1.3.6"></a>#06 `isAsciiDigit` [TODO]

通过将两个模式相减并检查结果是否为负数来实现比较运算.

#### <a id="1.3.7"></a>#07 `conditional` [TODO]

首先判断是否为零, 若为零则选择全 0 模式, 否则取反得到全 1 模式; 再使用按位与 `&` 进行条件选择.

#### <a id="1.3.8"></a>#08 `isLessOrEqual` [TODO]

先检查两个数是否同号, 若同号则通过相减是否得到负数进行判断, 若异号直接检查哪一个数为正哪一个数为负即可.

#### <a id="1.3.9"></a>#09 `logicalNeg` [TODO]

逻辑非等价于检查取反后的结果是否为全 0.

#### <a id="1.3.10"></a>#10 `howManyBits` [TODO]
...
#### <a id="2.2.2"></a>`bomb` - 主体

`bomb` 为可执行文件, 需要对其进行调试并破解得到 6 个密码.
...
- 测试通过的 6 个密码见文件[password.txt](2-bomb-lab/password.txt).
