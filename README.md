...
##### <a id="6.3.4"></a>`mm_free` 中忘记接住合并函数 `_coalesce` 的返回值
...
##### <a id="6.3.5"></a>放置函数 `_place` 中对不分割的情况的处理存在错误
...
##### <a id="6.3.6"></a>忘记在检查程序 `mm_check` 中释放内存
...
##### <a id="6.3.7"></a>测试样例 `binary-bal.rep` 和 `binary2-bal.rep` 的内存利用率过低
...
这么做除了性能低下 (最后两个测试样例的内存利用率分别为 28% 和 26%) 以外其他都挺好.
